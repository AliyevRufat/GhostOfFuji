...
VS_OUTPUT VS(VS_INPUT input)
{
	...
	for	(int i = 0; i < 4; ++i)
	{
		...
		float currentBoneIndex = input.boneIndex[i];
		//
		if (currentBoneIndex > -1) //the vertex is attached to a bone
		{
			transformedPosition +=  input.boneWeight[i] * mul(originalPosition, gBones[input.boneIndex[i]]); 	
			transformedPosition[3] = 1.0;
			transformedNormal +=  input.boneWeight[i] * mul(input.normal, gBones[input.boneIndex[i]]); 
		}
