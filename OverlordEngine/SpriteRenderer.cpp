...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		hr = gameContext.pDevice->CreateBuffer(&buffDesc, nullptr, &m_pVertexBuffer);
	}

	//TODO:
	// if the vertex buffer does not exists, or the number of sprites is bigger than the buffer size
	//		release the buffer
	...
	gameContext.pDevice->GetImmediateContext(&pDeviceContext);
	...
	if (m_pVertexBuffer)
	{
		...
		memcpy(mappedResource.pData, &m_Sprites[0], sizeof(VertexPosCol) * m_BufferSize);
		pDeviceContext->Unmap(m_pVertexBuffer, 0);
		pDeviceContext->Release();//recommended by docs.microsoft
	}
	//TODO: Fill Buffer
